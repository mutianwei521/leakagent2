"""
Resumable, time-guarded builder for the L-Town twin leak-response library.
Persists to data/ltown/leak_cache.pkl and SKIPS already-computed (node,rate)
entries, so it survives being killed: re-run until it prints COMPLETE. Exits
cleanly before the foreground time limit, saving progress.

Run repeatedly:  python data/ltown_build_library.py    (via wds_rag interpreter)
"""
import os
import sys
import time
...
CACHE = os.path.join(ROOT, "data", "ltown", "leak_cache.pkl")
LEAK_CANDS = 25
