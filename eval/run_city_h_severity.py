"""
City H controlled severity sweep: the companion to the register-anchored eval.
...
asks the complementary question: on the SAME City H network, at what leak size does
...
Run:  python eval/run_city_h_severity.py     (after the library build; via wds_rag python)
"""
import os
import sys
import json
...
def main():
    print("=" * 72)
    print(f"CITY_H CONTROLLED SEVERITY SWEEP (sensor_std={SENSOR_STD} m, {N_TEST} nodes/rate)")
    print("=" * 72)
    ...
    ctx.twin.load_cache(os.path.join(ROOT, "data", "city_h", "leak_cache.pkl"))
    ...
    out = {"provenance": {"network": "City H municipal WDN", "model": "nominal dataset/city_h.inp",
    ...
    json.dump(out, open(os.path.join(ROOT, "artifacts", "results_city_h_severity.json"), "w",
                        encoding="utf-8"), indent=2, ensure_ascii=False)
    print("\nsaved -> artifacts/results_city_h_severity.json")
    return 0
